...
### Batch MaxEnt with Robust Interpretive Parsing (see Section 3.2.2.1)

  # See the file "MaxEntLearning_3.R" (see under the folder "Simple Lexical Stress") for the following functions, on which the functions in this file are dependent.
  ...
  # 5. get_plog(): obtain the PLOG (log probability) of each candidate, multipled by its frequency, and then summed. This is the "objective function" used in batch processing of data.
    # Dependent on the function get_cand_prob() above.
...
## From this point: working with the peninitial stress grammar  
...
get_compatibles <- function(OT_Tableau, overt_forms){ #Assumes an OT-Soft-style tableau with candidates consisting of overt form --> parse pairs, and a two-column table object in which the left column contains overt forms with given frequencies.
  ...
  for(i in 1:(number_combinations*1000)){ # I generated the combinations in a Monte Carlo Fashion, because I was too lazy to enumerate them analytically.
    temp_combination <- c()
...
## Examine element 2 of my_compatibles for the negative log-likelihood of the grammar with a particular combination of parses.
## Examine element 3 of my_compatibles for whether the weights learned were convergent (1 = no).




###### BELOW HERE: Testing / Development code, not to be used.
log_likelihoods <- c()
